...
async def _callback_game_group(
    callback: CallbackQuery, session: AsyncSession, *, game_id: int
    ...
    if (
        ...
        await callback.answer(
            "❌ Эта кнопка относится к другой игровой сессии.", show_alert=True
        )
        return None
    return game, group
...
async def _can_start_lobby(
    bot: Bot, session: AsyncSession, *, group: Group, game: GameSession, user_id: int
) -> bool:
    if user_id == game.creator_telegram_id or await can_manage_group(
        bot, group, user_id, session
    ):
        return True
    ...
    return policy == "any_at_min" and await _is_joined_player(
        session, game_id=game.id, user_id=user_id
    )


def _games_markup() -> InlineKeyboardMarkup:
    buttons = [
        InlineKeyboardButton(text=d.title, callback_data=f"gm:new:{d.code}")
        for d in game_registry.all()
    ]
...
async def games_command(message: Message, bot: Bot, session: AsyncSession) -> None:
    ...
    if group is None:
        await message.reply(
            "🎮 Игровой центр доступен только в группах, подключённых к Mimoru.\n"
            "Владелец группы должен отправить команду «подключить»."
        )
        return
    await send_game_center_snapshot(
        bot, session, group=group, reply_to_message_id=message.message_id
    )
...
async def game_profile(callback: CallbackQuery, session: AsyncSession) -> None:
    ...
    if group is None:
        ...
        return
    name = (
        callback.from_user.full_name or callback.from_user.username or "Игрок"
    ).strip()
    await callback.answer(
        await render_profile(
            session, group_id=group.id, user_id=callback.from_user.id, name=name
        ),
        show_alert=True,
    )
...
async def game_rating(callback: CallbackQuery, session: AsyncSession) -> None:
    ...
    await callback.message.edit_text(
        await render_rating(session, group_id=group.id), reply_markup=markup
    )
    await callback.answer()
...
async def game_specific_rules(callback: CallbackQuery) -> None:
    ...
    if definition is None:
        ...
        return
    await callback.answer(
        f"{definition.title}: правила доступны после выбора игры.", show_alert=True
    )


@router.callback_query(F.data.regexp(r"^gm:new:[a-z0-9_]{1,32}$"))
async def game_create_lobby(
    ...
    if settings is not None and not settings.enabled:
        await callback.answer(
            "🎮 Игры в этой группе отключены администратором.", show_alert=True
        )
        return
    if settings is not None and settings.allowed_games and code not in settings.allowed_games:
        await callback.answer(
            "Эта игра отключена в настройках группы.", show_alert=True
        )
        return
    name = (
        callback.from_user.full_name or callback.from_user.username or "Игрок"
    ...
    try:
        ...
        title = (
            game_registry.get(active.game_type).title
            if active and game_registry.get(active.game_type)
            else "другая игра"
        )
        await callback.answer(
...
async def game_join_lobby(
    ...
    try:
        ...
        await callback.answer(
            (
                "Лобби уже заполнено."
                if "full" in str(error)
                else "Присоединиться уже нельзя."
            ),
            show_alert=True,
        )
        return
...
async def game_leave_lobby(
    ...
    try:
        await manager.leave_lobby(
            session, game_id=game.id, user_telegram_id=callback.from_user.id
        )
    except GamePlayerError as error:
        await callback.answer(
            (
                "Создатель лобби может только отменить игру."
                if "creator" in str(error)
                else "Выйти из этого лобби уже нельзя."
            ),
            show_alert=True,
        )
        return
...
async def game_leave_running(
    ...
    if game.status not in {
        ...
        return
    try:
        result = await manager.leave_running(
            session, game_id=game.id, user_telegram_id=callback.from_user.id
        )
    except GamePlayerError as error:
    ...
    if result == LeaveResult.CANCELLED:
        await retire_active_messages(
            ...
            session,
            game_id=game.id,
            replacement_text="❌ Игра отменена: создатель вышел.",
        )
...
async def game_start_lobby(
    ...
    if not await _can_start_lobby(
        bot, session, group=group, game=game, user_id=callback.from_user.id
    ):
        await callback.answer(
            "❌ У вас нет права запускать это лобби.", show_alert=True
        )
        return
    try:
        ...
        if "not enough" in str(error):
            await callback.answer(
                f"Нужно минимум {game_registry.require(game.game_type).min_players} игроков.",
                show_alert=True,
            )
        else:
    ...
    try:
        ...
        await callback.answer(
            "Игра сохранена для восстановления после ошибки запуска.", show_alert=True
        )
        return
    ...
    await ensure_game_panel(bot, session, group=group, pin=False)
    log.info(
        "game_started", game_id=game.id, group_id=group.id, game_type=game.game_type
    )
...
async def game_cancel(callback: CallbackQuery, bot: Bot, session: AsyncSession) -> None:
    ...
    if game.status not in {
        ...
        return
    can_cancel = (
        callback.from_user.id == game.creator_telegram_id
        or await can_manage_group(bot, group, callback.from_user.id, session)
    )
    if not can_cancel:
        await callback.answer(
            "❌ Отменить игру может создатель или администратор.", show_alert=True
        )
        return
        game = await manager.cancel_game(
            session, game_id=game.id, reason="cancelled_by_user"
        )
...
async def game_open(callback: CallbackQuery, bot: Bot, session: AsyncSession) -> None:
    ...
    if game.status == GameSessionStatus.LOBBY.value:
        await ensure_lobby_message(
            bot, session, group=group, game=game, manager=manager
        )
    ...
    if sync_ui is not None:
        try:
            ...
            log.exception(
                "game_open_sync_failed", game_id=game.id, game_type=game.game_type
            )
            await callback.answer(
                "Не удалось открыть карточку игры. Попробуйте ещё раз.", show_alert=True
            )
            return
